...
public interface ITextGenerationService : IDisposable
{
    ...
    /// <remarks>Yields <see cref="StreamChunk.Content"/> and optionally <see cref="StreamChunk.Reasoning"/>; never tool calls.</remarks>
    IAsyncEnumerable<StreamChunk> GenerateStreamingAsync(AIModelManifest manifest, string prompt, CancellationToken ct, IReadOnlyList<string>? imageBase64Contents = null);
    /// <summary>
    /// Tool-aware streaming generation. Yields <see cref="StreamChunk.Content"/> / <see cref="StreamChunk.Reasoning"/> during generation
