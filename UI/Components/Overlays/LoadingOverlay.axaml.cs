...
namespace MnemoApp.UI.Components.Overlays
{
    public partial class LoadingOverlay : UserControl, INotifyPropertyChanged
    {
        ...
        public LoadingOverlay(ITaskSchedulerService taskScheduler) : this()
        {
        ...
        private void OnTaskPropertyChanged(object? sender, PropertyChangedEventArgs e)
        {
            Dispatcher.UIThread.Post(() =>
            {
                ...
                // Auto-close overlay when task finishes (Completed/Failed/Cancelled)
                        if (_task != null && (_task.Status == TaskStatus.Completed || _task.Status == TaskStatus.Failed || _task.Status == TaskStatus.Cancelled))
                        {
        ...
        public string StatusText
        {
            get
            {
                return _task.Status switch
                {
                    ...
                    TaskStatus.Running => $"{_task.Progress:P0}",
